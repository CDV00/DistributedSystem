...
public class Product : AggregateRoot<Guid>, IAuditableEntity {
    ...
    public static Product CreateProduct(Guid id, string name, decimal price, string description) {
        ...
        product.RaiseDomainEvent(new Contract.Services.V1.Product.DomainEvent.ProductCreated(Guid.NewGuid(), product.Id, product.Name, product.Price, product.Description));
